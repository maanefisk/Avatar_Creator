...
function category(chosencategory) {
  ...
  if (
    activecategory == "hair" ||
    activecategory == "ears" ||
    activecategory == "eyelids" ||
    activecategory == "nose" ||
    activecategory == "mouth" ||
    activecategory == "lips" ||
    activecategory == "brows"
  ) {
    slider.value = 0;
...
function thenumber(chosennumber) {
  ...
  for (nuli = 0; nuli < model.boldnumbers.length; nuli++) {
    if (
      model.boldnumbers[nuli].classList[1] == chosennumber.classList[1] &&
      model.boldnumbers[nuli] != chosennumber
    ) {
...
function changingcolor(thecolorpicker) {
  ...
  if (chosenelement == "" || categoryathand == "background") {
    document.getElementById("background1").style.fill =
      thecolorpicker.style.background ||
      thecolorpicker.value ||
      thecolorpicker.getAttribute("fill");
    ...
    for (x = 1; x <= eval(activecategory); x++) {
      ...
      if (allincategory.children.length != 0) {
        for (o = 0; o < allincategory.children.length; o++) {
          allincategoryinner[o].setAttribute(
            "style",
            "fill:" + thecolorpicker.value + ";"
          );
          allincategoryinner[0].setAttribute(
            "style",
...
function generatenumbersview(activeone) {
  ...
  if (
    ...
    textofnumbers +=
      '<button id="xout' +
      n +
      '" class="number xout" onclick="removestyle()">' +
...
function generategradientssview(activecategorynowforgradient) {
  ...
  for (catego = 0; catego < model.categories.length; catego++) {
    ...
    if (activecategorynowforgradient == model.categories[catego]) {
      textofpalette9 +=
        '<div class="paletteheader">Gradient ' +
        gradientcategory +
        "</div>\n" +
        "\n" +
        ...
        '                        <circle pointer-events="all" class="gradientchooser" onclick="changingcolor(this)" fill="url(#gradient1' +
        activecategory +
        ')"/>\n' +
        '                        <circle pointer-events="all" class="gradientchooser" onclick="changingcolor(this)" fill="url(#gradient2' +
        activecategory +
        ')"/>\n' +
        '                        <circle pointer-events="all" class="gradientchooser" onclick="changingcolor(this)" fill="url(#gradient3' +
        activecategory +
        ')"/>\n' +
        '                        <circle pointer-events="all" class="gradientchooser" onclick="changingcolor(this)" fill="url(#gradient4' +
        activecategory +
        ')"/>\n' +
        '                        <circle pointer-events="all" class="gradientchooser" onclick="changingcolor(this)" fill="url(#gradient5' +
        activecategory +
        ')"/>\n' +
        "                    </svg>\n" +
        "                </div>";
  ...
  if (typeof butt != "undefined") {
    for (j = 0; j < model.palettes.length; j++) {
      if (visiblepalette == model.palettes[j]) {
        if (
          butt == rightarrow &&
          activenow != "palette" + model.palettes.length
        ) {
